...
class Losses(nn.Module):
    ...
    def forward(self, sample_list: Dict[str, Tensor], model_output: Dict[str, Tensor]):
        ...
        if "gt_targets" not in sample_list:
            if not self._evaluation_predict:
                warnings.warn(
                    "Sample list has not field 'gt_targets', are you "
                    "sure that your ImDB has labels? you may have "
                    "wanted to run with evaluation.predict=true"
                )
            return output
...
@registry.register_loss("label_smooth_cross_entropy")
class LabelSmoothCrossEntropyLoss(nn.Module):
    ...
    def compute_loss(self, targets, logits):
        ...
        num_ray = targets.size(1)
        targets = targets.repeat(1,num_pts//num_ray).reshape(-1, 1)

        weight = logits.new_tensor(
            [1. for _ in range(batch_size * num_pts)])
    ...
    def forward(self, sample_list, model_output):
        try:
            loss = self.compute_loss(sample_list['gt_targets'], model_output["acc_seg"])
        except:
            print("x")
            exit(-1)
        return loss
