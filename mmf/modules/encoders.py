...
from mmf.modules.resnet_head import StageSpec, ResNetHead
...
class LocFeatureEncoder(Encoder):
    @dataclass
...
class ImageFeatureEncoderFactory(EncoderFactory):
    ...
    def __init__(self, config: Config, *args, **kwargs):
        ...
        elif encoder_type == "resnet_head":
            self.module = ResNetHeadImageEncoder(params)
        else:
...
@registry.register_encoder("resnet_head")
class ResNetHeadImageEncoder(ImageFeatureEncoder):
    @dataclass
    class Config(Encoder.Config):
        neme: str = "resnet_head"
        in_dim: int = MISSING
        config: dict = None
        weights_file: str = "resnet_head.pth"
        model_data_dir: str = MISSING
    ...
    def __init__(self, config: Config, *args, **kwargs):
        ...
        head = ResNetHead(
            ...
            stride_init=None,
            res2_out_channels=config.RES2_OUT_CHANNELS,
            dilation=config.RES5_DILATION
        )
        self.head = head
        self.out_dim = head.out_channels

        pre_weights = torch.load(weights_file)
        layer_name = []
        for name in self.head.state_dict():
            layer_name.append(name)
        dt = OrderedDict()
        for i, (_, value) in enumerate(pre_weights.items()):
            dt[layer_name[i]] = value
        self.head.load_state_dict(dt)
