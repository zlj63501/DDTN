...
class ClassifierLayer(nn.Module):
    def __init__(self, classifier_type, in_dim, out_dim, **kwargs):
        ...
        elif classifier_type == "twin_linear":
            self.module = TwinLinear(in_dim, out_dim, **kwargs)
...
class ModalCombineLayer(nn.Module):
    def __init__(self, combine_type, img_feat_dim, txt_emb_dim, **kwargs):
        ...
        elif combine_type == "loc_img_text_element_multiply":
            self.module = LocVqaMultiply(img_feat_dim, txt_emb_dim, **kwargs)
        else:
...
class LocVqaMultiply(nn.Module):
    def __init__(self, image_feat_dim, ques_emb_dim, **kwargs):
        super().__init__()
        self.out_dim = 5000
        self.linear_sga = ReLUWithWeightNormFC(image_feat_dim, self.out_dim)
        self.linear_ques = ReLUWithWeightNormFC(ques_emb_dim, self.out_dim)
        self.dropout = nn.Dropout(0.1)

    def forward(self, vqa_embedding, q):
        joint_vqa = self.linear_sga(vqa_embedding) * self.linear_ques(q)
        joint_vqa = self.dropout(joint_vqa)
        return joint_vqa
...
class NonLinearElementMultiply(nn.Module):
    ...
    def forward(self, image_feat, question_embedding, context_embedding=None):
        ...
        joint_feature = image_fa * question_fa_expand # 直接相乘
...
class NonLinearDepressMultiply(nn.Module):
    def __init__(self, image_feat_dim, ques_emb_dim, **kwargs):
        super().__init__()
        self.fa_image = ReLUWithWeightNormFC(image_feat_dim, kwargs["hidden_dim"])
        self.fa_txt = ReLUWithWeightNormFC(ques_emb_dim, kwargs["hidden_dim"])
    ...
    def forward(self, image_feat, question_embedding, bbox, context_embedding=None):
        image_fa = self.fa_image(image_feat)
...
class TwinLinear(nn.Module):
    ...
    def __init__(self, in_dim: int, out_dim: int, **kwargs):
        ...
        self.linear_vqa = nn.Linear(in_dim, out_dim)
        # self.linear_seg = nn.ModuleList([nn.Linear(in_dim, num_bin) for _ in range(3)])
        self.linear_seg = nn.Linear(in_dim, num_bin)

    def forward(self, vqa_feat: torch.Tensor, seg_feat: torch.Tensor) -> torch.Tensor:
        vqa_feat = self.linear_vqa(vqa_feat)
        seg_feat = self.linear_seg(seg_feat)
        return vqa_feat, seg_feat
...
class BranchCombineLayer(nn.Module):
    ...
    def forward(
        self, v_cga: torch.Tensor) -> torch.Tensor:

        feat_vqa = [self.layer_norm(self.linear_ques(v_cga))]
        return feat_vqa
