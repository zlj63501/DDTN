...
@registry.register_metric("seg_accuracy")
class seg_accuracy(BaseMetric):
    def __init__(self):
        super().__init__("seg_accuracy")
        self.mapping = 'relative'
        self.required_params = ["acc_seg", "gt_mask_rle", "pad_shape"]

    def prepare_mask(self, sample_list, pred_masks, rescale=False):
        seq_out_mask = torch.argmax(pred_masks, dim=2)
        self.end = pred_masks.size(2)
        seq_out_mask = seq_out_mask.cpu().numpy()
        pad_shapes = sample_list["pad_shape"].cpu().numpy()
        pred_masks = []
        for pred_mask, pad_shape in zip(seq_out_mask, pad_shapes):
            h_pad, w_pad = pad_shape[:2]
            ends = numpy.argwhere(
                pred_mask == self.end)[:, 0]
            if not len(ends) == 0:
                pred_mask = pred_mask[:ends[0]]
            if len(pred_mask) % 2 != 0:
                pred_mask = pred_mask[:-1]

            scale_factor = [[w_pad, h_pad]
                            for _ in range(len(pred_mask) // 2)]
            scale_factor = numpy.array(scale_factor).reshape(-1)
            pred_mask = self.dequantize(pred_mask, scale_factor)
            ...
            pred_rles = maskUtils.frPyObjects(
                pred_mask, h_pad, w_pad)
            pred_rle = maskUtils.merge(pred_rles)

            if rescale:
                h_img, w_img = sample_list.height, sample_list.wdith
    ...
    def dequantize(self, seq, scale_factor):
        if self.mapping == "relative":
            return seq * scale_factor / self.end
        elif self.mapping == "absolute":
            return seq * 640. / self.end

    def mask_overlaps(self, gt_mask, pred_masks):
        """Args:
            gt_mask (list[RLE]):
            pred_mask (list[RLE]):
        """
        ...
        is_crowd = [0 for _ in range(len(gt_mask))]
        mask_iou = computeIoU_RLE(gt_mask, pred_masks, is_crowd)
        mask_iou = torch.from_numpy(mask_iou)

        return mask_iou

    def calculate(self, sample_list, model_output, device="cuda:0"):
        pred_masks = model_output['acc_seg']
        pred_masks = self.prepare_mask(sample_list, pred_masks)

        gt_mask = sample_list["gt_mask_rle"]
        gt_masks = []
        for size, counts in zip(gt_mask.size, gt_mask.counts):
            gt_masks.append({"size": size, "counts": counts})
        eval_mask = pred_masks is not None

        mask_iou = torch.tensor([-1.], device=device)
        mask_acc_at_thrs = torch.full((10, ), -1., device=device)
        if eval_mask:
            mask_iou = self.mask_overlaps(gt_masks, pred_masks).to(device)
            for i, iou_thr in enumerate([x*0.05 for x in range(10,20)]):
                mask_acc_at_thrs[i] = (
                    mask_iou >= iou_thr).float().mean()

            mask_iou = torch.mean(mask_iou).item()
            mask_acc = mask_acc_at_thrs * 100.
            mask_acc_avg = torch.mean(mask_acc_at_thrs).item() * 100.

        return {"IOU_mean": mask_iou * 100.,
                "mask_acc_0.5": mask_acc[0],
                "mask_acc_0.75": mask_acc[5],
                "mask_acc": mask_acc_avg,
                }
