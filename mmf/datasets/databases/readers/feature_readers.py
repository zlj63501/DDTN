...
def load_features(feat_path: str):
    if not os.path.exists(feat_path):
        feat_path = feat_path.replace('test', 'val')
...
class FeatureReader:
    ...
    def read(self, image_feat_path):
        if not image_feat_path.endswith("npy") and not image_feat_path.endswith("npz") and not image_feat_path.endswith("pth"):
            return None
        image_feat_path = os.path.join(self.base_path, image_feat_path).replace('.jpg.npz', '.npz')
        info = {}
        ...
        feat = load_features(image_feat_path)
        info['image_id'] = int(image_feat_path.split('/')[-1].split('_')[-1].split('.')[0])
        info['image_height'] = feat['ori_shape'][1]
        info['image_width'] = feat['ori_shape'][0]
        info['pad_height'] = feat['img_shape'][1]
        info['pad_width'] = feat['img_shape'][0]
        if 'grid_feat' in feat.files:
            info['grid_feat'] = feat['grid_feat']
        if 'bbox' in feat.files:
            info['prefinded_bbox'] = feat["bbox"]
        feat = feat['X']
        self.ndim = feat.shape[0]

        return torch.from_numpy(feat.squeeze()), info
